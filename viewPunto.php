...
$result = mysqli_query($mysqli, "SELECT * FROM products where inventario = 'Punto estudiantil'");
if (isset($result)) {
...
<body>
    ...
    <?php
        if(!$_GET){
            header('location:viewPunto.php?pagina=1');
        }
        ;
    ?>
    ...
    <?php
     ...
     $result2 = mysqli_query($mysqli, "SELECT * FROM products where inventario = 'Punto estudiantil' LIMIT $iniciar,$material_x_pagina");
  ...
  <li class="page-item <?php echo $_GET['pagina']<=1? 'disabled' : '' ?>
    ">
        <a class="page-link" href="viewPunto.php?pagina=<?php echo $_GET['pagina']-1 ?>">
            Anterior
        </a>
    </li>
    <?php for ($i=0; $i < $pagina; $i++) :?>
    <li class="page-item <?php echo $_GET['pagina']==$i+1 ? 'active' : '' ?>">
        <a class="page-link" href="viewPunto.php?pagina=<?php echo $i+1 ?>">
            <?php echo $i+1 ?>
        </a>
    </li>
    <?php endfor ?>
