...
$result = mysqli_query($mysqli, "SELECT * FROM products where login_id = '$idUser'");
if (isset($result)) {
...
<body>
    <?php
        if(!$_GET){
            header('location:viewMaterialUser.php?pagina=1');
        }
        ;
    ?>
    ...
    $result2 = mysqli_query($mysqli, "SELECT * FROM products  where login_id = '$idUser' LIMIT $iniciar,$material_x_pagina");
...
<nav class="mt-5 ml-5" aria-label="Page navigation example">
  ...
  <li class="page-item <?php echo $_GET['pagina']<=1? 'disabled' : '' ?>
    ">
        <a class="page-link" href="viewMaterialUser.php?pagina=<?php echo $_GET['pagina']-1 ?>">
            Anterior
        </a>
    </li>
    <?php for ($i=0; $i < $pagina; $i++) :?>
    <li class="page-item <?php echo $_GET['pagina']==$i+1 ? 'active' : '' ?>">
        <a class="page-link" href="viewMaterialUser.php?pagina=<?php echo $i+1 ?>">
            <?php echo $i+1 ?>
        </a>
    </li>
    <?php endfor ?>
